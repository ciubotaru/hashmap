...
static void test_delete_null_map_pointer(void) {
	int key = 42;
	assert(hash_delete(NULL, &key, sizeof(key)) == -1);
}

static void test_delete_null_map(void) {
	hashmap_t *map = NULL;
	int key = 42;
	assert(hash_delete(&map, &key, sizeof(key)) == -1);
	assert(map == NULL);
}
...
static void test_delete_missing_key(void) {
	...
	assert(hash_delete(&map, &missing_key, sizeof(missing_key)) == -1);
	assert(map != NULL);
...
static void test_delete_one_of_multiple_entries(void) {
	...
	assert(hash_probe(map, &keys[0], sizeof(keys[0])) == 1);
	assert(hash_probe(map, &keys[1], sizeof(keys[1])) == 0);
	assert(hash_probe(map, &keys[2], sizeof(keys[2])) == 1);
	hashmap_clear(&map);
	assert(map == NULL);
}
...
static void test_delete_all_entries(void) {
	...
	for (size_t i = 0; i < 3; i++) {
		assert(hash_delete(&map, &keys[i], sizeof(keys[i])) == 0);
	}
	assert(map == NULL);
}
...
static void test_delete_null_key_with_zero_size(void) {
	...
	assert(hash_delete(&map, NULL, 0) == 0);
	assert(map == NULL);
}
...
static void test_delete_nonnull_key_with_zero_size(void) {
	...
	int other_key = 1234;
	int data = 5678;
	assert(hash_insert(&map, &key, 0, &data, sizeof(data)) == 0);
	assert(hash_delete(&map, &other_key, 0) == 0);
	assert(map == NULL);
}
...
static void test_delete_null_key_with_nonzero_size(void) {
	...
	assert(hash_insert(&map,
			   &key, sizeof(key), &data, sizeof(data)) == 0);
	assert(hash_delete(&map, NULL, sizeof(key)) == -1);
	assert(map != NULL);
	assert(hash_probe(map, &key, sizeof(key)) == 1);
	hashmap_clear(&map);
	assert(map == NULL);
...
static void test_delete_then_insert(void) {
	...
	assert(hash_delete(&map, &key1, sizeof(key1)) == 0);
	assert(map == NULL);
	assert(hash_insert(&map,
	...
	assert(hashmap_info(map, &info) == 0);
	assert(info.nr_entries + info.nr_entries_old == 1);
	assert(hash_probe(map, &key2, sizeof(key2)) == 1);
	hashmap_clear(&map);
	assert(map == NULL);
}
