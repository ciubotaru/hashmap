...
static void test_put_null_map_pointer(void) {
	int key = 42;
	int data = 1234;
	assert(hash_put(NULL, &key, sizeof(key), &data, sizeof(data)) == -1);
}
...
static void test_put_into_null_map(void) {
	...
	int data = 1234;
	hashmap_info_t info;
	assert(hash_put(&map, &key, sizeof(key), &data, sizeof(data)) == 0);
	assert(map != NULL);
...
static void test_put_missing_key_inserts(void) {
	...
	assert(hash_search(map,
			   &key, sizeof(key), &found_data, &found_size) == 0);
...
static void test_put_existing_key_updates(void) {
	...
	assert(hash_put(&map,
			&key, sizeof(key), &new_data, sizeof(new_data)) == 0);
	assert(hash_search(map,
			   &key, sizeof(key), &found_data, &found_size) == 0);
...
static void test_put_multiple_entries(void) {
	...
	for (size_t i = 0; i < 3; i++) {
		assert(hash_probe(map, &keys[i], sizeof(keys[i])) == 1);
	}
	hashmap_clear(&map);
	assert(map == NULL);
}
...
static void test_put_zero_size_key(void) {
	...
	const void *found_data;
	size_t found_size;
	assert(hash_put(&map, &key, 0, &old_data, sizeof(old_data)) == 0);
	assert(hash_put(&map,
			&other_key, 0, &new_data, sizeof(new_data)) == 0);
	assert(hashmap_info(map, &info) == 0);
	assert(info.nr_entries == 1);
	assert(hash_search(map, NULL, 0, &found_data, &found_size) == 0);
	assert(found_size == sizeof(new_data));
	assert(memcmp(found_data, &new_data, sizeof(new_data)) == 0);
	hashmap_clear(&map);
	assert(map == NULL);
...
static void test_put_zero_size_data(void) {
	...
	size_t found_size;
	assert(hash_put(&map,
			&key, sizeof(key), &old_data, sizeof(old_data)) == 0);
	assert(hash_put(&map, &key, sizeof(key), NULL, 0) == 0);
	assert(hash_search(map,
			   &key, sizeof(key), &found_data, &found_size) == 0);
...
static void test_put_nonnull_data_with_zero_size(void) {
	...
	assert(hash_put(&map,
			&key, sizeof(key), &old_data, sizeof(old_data)) == 0);
	assert(hash_put(&map, &key, sizeof(key), &ignored_data, 0) == 0);
	assert(hash_search(map,
			   &key, sizeof(key), &found_data, &found_size) == 0);
	assert(found_data == NULL);
	assert(found_size == 0);
...
static void test_put_null_key_with_nonzero_size(void) {
	hashmap_t *map = NULL;
	int data = 1234;
	assert(hash_put(&map, NULL, sizeof(int), &data, sizeof(data)) == -1);
	assert(map == NULL);
}
...
static void test_put_null_data_with_nonzero_size(void) {
	hashmap_t *map = NULL;
	int key = 42;
	assert(hash_put(&map, &key, sizeof(key), NULL, sizeof(int)) == -1);
	assert(map == NULL);
}
...
static void test_put_copies_data_on_update(void) {
	...
	size_t found_size;
	assert(hash_put(&map,
			&key, sizeof(key), &(int) { 1 }, sizeof(int)) == 0);
	assert(hash_put(&map, &key, sizeof(key), &data, sizeof(data)) == 0);
	data = 5678;
	assert(hash_search(map,
			   &key, sizeof(key), &found_data, &found_size) == 0);
